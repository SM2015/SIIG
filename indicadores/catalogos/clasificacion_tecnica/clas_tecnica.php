<?php
function agregar_clas_tecnica(){
$cnx=bdd_conectar(); ?>
<form action="<?php echo $_SERVER['PHP_SELF']?>?action=guardarnuevo" method="POST" id="add" name="add">
<h3 class="frm-title-x">Agregar una Categoria de clasificacion tecnica</h3>
	<fieldset ><legend> Ingreso de una Categoria de clasificacion tecnica</legend>
...
frmvalidator.addValidation("descripcion","req","Nombre de la Categoria de clasificacion tecnica es requerida");
</script>
 <?php
bdd_cerrar($cnx);
}

function grabar_nuevo_clas_tecnica(){
$cnx = bdd_conectar();
$descripcion =(isset($_POST['descripcion'])) ? $_POST['descripcion'] : "";
$comentario =(isset($_POST['comentario'])) ? $_POST['comentario'] : "";

$q = "INSERT INTO clasificacion_tecnica ( descripcion, comentario )
    VALUES (
  '".$descripcion."' ,
  '".$comentario."')";
	...
	if ($af){ 
		...
		<p > Regresar a <a href="/indicadores/catalogos/clasificacion_tecnica/">Clasificacion Tecnica</a></p>
		<?php
	} else { 
		?>
	...
	if ($rs) { 
		...
		if ($num > 0) { 
			...
			while ($i < $numf  ) {
				...
				$fname = $meta;
				switch ($fname) { //, descripcion, comentario
...
function editar_clas_tecnica($id){
	$cnx = bdd_conectar();
	$q = '	SELECT id_clasificacion_tecnica, descripcion, comentario
  FROM clasificacion_tecnica where id_clasificacion_tecnica='.$id;
	$rs = bdd_pg_query($cnx, $q);
	if ($rs) { 
		$reg = bdd_pg_fetch_row($rs)
		?>
		<h3>Editar una Categoria de clasificacion tecnica</h3>
		<form action="<?php echo $_SERVER['PHP_SELF']?>?action=guardar" method="POST" id="edit" name="edit" >
			<fieldset ><legend> Editar una Categoria de clasificacion tecnica </legend>
	<table width="100%" align="center">
		<TR class="frm-fld-x-odd">
			<TD width="20%">Categoria de clasificacion</TD>
			<TD width="80%">
...
function actualizar_clas_tecnica($id){
	$cnx = bdd_conectar();
$descripcion =(isset($_POST['descripcion'])) ? $_POST['descripcion'] : "";
$comentario =(isset($_POST['comentario'])) ? $_POST['comentario'] : "";
$q = "UPDATE clasificacion_tecnica SET
			descripcion= 	'".$descripcion."'
		,	comentario= 	'".$comentario."'
		where 	id_clasificacion_tecnica=".$id;
	$rs = bdd_pg_query($cnx, $q);
	$af = bdd_pg_affected_rows($rs);
	if ($af){ 
		?>
...
function borrar_clas_tecnica($id){
	if ($id == NULL) {
		?>
		...
		$cnx = bdd_conectar();
                $q = "DELETE FROM clasificacion_tecnica WHERE id_clasificacion_tecnica =".$id;
                $rs = @bdd_pg_query($cnx, $q);
                if ($rs){
                    $num=@bdd_pg_num_rows($rs);
                        ?>
