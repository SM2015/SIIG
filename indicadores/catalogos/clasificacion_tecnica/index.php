...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        <title>Clasificacion Tecnica de Indicadores</title>
	...
	<table  border="0" cellpadding="2" cellspacing="0" class="dataTable" width="100%">
        ...
        switch ($accion) {
                case 'nuevo':
                        agregar_clas_tecnica();
                break;
                case 'guardarnuevo':
                        grabar_nuevo_clas_tecnica();
                break;
                case 'editar':
                        $id = (isset($_REQUEST['id']))? $_REQUEST['id'] : NULL;
                        editar_clas_tecnica($id);
                break;
                case 'guardar':
                        $id = (isset($_REQUEST['id']))? $_REQUEST['id'] : NULL;
                        actualizar_clas_tecnica($id);
                break;
                        case 'borrar':
                        $id = (isset($_REQUEST['id']))? $_REQUEST['id'] : NULL;
                        borrar_clas_tecnica($id);
                break;
                default: ?>
                        <h2 align="center"><a href="index.php?action=nuevo" title="Agregar una Clasificacion Tecnica">Agregar una Clasificacion Tecnica</a></h2>	<?php
                        $data = array('Editar','Borrar');
                        $urls = array('Editar','Borrar');
                        echo listarTodos('clasificacion_tecnica', $data, $urls, 'id_clasificacion_tecnica, descripcion');
                break;
        }?>
    </body>
</html>
