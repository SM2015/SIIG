<?php
session_start();
include_once("clas_privacidad.php");
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        <title>Clasificacion del Indicador segun su privacidad</title>
	...
	<table  border="0" cellpadding="2" cellspacing="0" class="dataTable" width="100%">
        ...
        switch ($accion) {
                case 'nuevo':
                        agregar_clas_privacidad();
                break;
                case 'guardarnuevo':
                        grabar_nuevo_clas_privacidad();
                break;
                case 'editar':
                        $id = (isset($_REQUEST['id']))? $_REQUEST['id'] : NULL;
                        editar_clas_privacidad($id);
                break;
                case 'guardar':
                        $id = (isset($_REQUEST['id']))? $_REQUEST['id'] : NULL;
                        actualizar_clas_privacidad($id);
                break;
                        case 'borrar':
                        $id = (isset($_REQUEST['id']))? $_REQUEST['id'] : NULL;
                        borrar_clas_privacidad($id);
                break;
                default: ?>
                        <h2 align="center"><a href="index.php?action=nuevo" title="Agregar una Clasificacion segun su privacidad">Agregar una Clasificacion segun su privacidad</a></h2>	<?php
                        $data = array('Editar','Borrar');
                        $urls = array('Editar','Borrar');
                        echo listarTodos('clasificacion_privacidad', $data, $urls, 'id_clasificacion_privacidad, descripcion');
                break;
        }?>
    </body>
</html>
