<?php
function agregar_responsable_dato(){
$cnx=bdd_conectar(); ?>
<form action="<?php echo $_SERVER['PHP_SELF']?>?action=guardarnuevo" method="POST" id="add" name="add">
<h3 class="frm-title-x">Agregar un Responsable de Dato</h3>
	<fieldset ><legend> Ingreso de un Responsable de Dato</legend>
	<table width="100%" align="center">
		<TR class="frm-fld-x-odd">
			<TD width="20%">Nombre del Responsable del Dato</TD>
			<TD width="80%">
...
function grabar_nuevo_responsable_dato(){
$cnx = bdd_conectar();
...
$q = "INSERT INTO responsable_dato ( nombre, contacto, correo, telefono, cargo )
    VALUES (
  '".$nombre."' ,
  '".$contacto."' ,
  '".$correo."' ,
  ...
  '". $cargo ."')";
	...
	if ($af){ 
		...
		<p > Regresar a <a href="/indicadores/catalogos/responsable_dato/">Responsable de Datos</a></p>
		<?php
	} else { 
		?>
	...
	if ($rs) { 
		...
		if ($num > 0) { 
			...
			while ($i < $numf  ) {
				...
				$fname = $meta;
				switch ($fname) {
					case 'id_responsable_dato':
						$fname = "ID";
					break;
					case 'nombre':
						$fname = "Nombre del establecimiento o dependencia responsable del dato";
					break;
					case 'contacto':
...
function editar_responsable_dato($id){
	$cnx = bdd_conectar();
	$q = '	SELECT id_responsable_dato, nombre, contacto, correo, telefono, cargo
  FROM responsable_dato where id_responsable_dato='.$id;
	$rs = bdd_pg_query($cnx, $q);
	if ($rs) { 
		$reg = bdd_pg_fetch_row($rs)
		?>
		<h3>Editar Un Responsable de Dato</h3>
		<form action="<?php echo $_SERVER['PHP_SELF']?>?action=guardar" method="POST" id="edit" name="edit" >
			<fieldset ><legend> Editar Un responsable de Dato </legend>
	<table width="100%" align="center">
		<TR class="frm-fld-x-odd">
			<TD width="20%">Nombre del Responsable del Dato</TD>
			<TD width="80%">
...
function actualizar_responsable_dato($id){
	$cnx = bdd_conectar();
	$nombre =(isset($_POST['nombre'])) ? $_POST['nombre'] : "";
	$contacto =(isset($_POST['contacto'])) ? $_POST['contacto'] : "";
	$correo =(isset($_POST['correo'])) ? $_POST['correo'] : "";
	$telefono =(isset($_POST['telefono'])) ? $_POST['telefono'] : "";
	$cargo =(isset($_POST['cargo'])) ? $_POST['cargo'] : "";
	$q = "UPDATE responsable_dato SET
			nombre= 	'".$nombre."'
		,	contacto= 	'".$contacto."'
		,	correo= 	'".$correo."'
		,	telefono= 	'".$telefono."'
		,	cargo= 		'".$cargo."'
		where 	id_responsable_dato=".$id;
	$rs = bdd_pg_query($cnx, $q);
	$af = bdd_pg_affected_rows($rs);
	if ($af){ 
		?>
...
function borrar_responsable_datos($id){
	if ($id == NULL) {
		?>
		...
		$cnx = bdd_conectar();
                $q = "DELETE FROM responsable_dato WHERE id_responsable_dato=".$id;
                $rs = @bdd_pg_query($cnx, $q);
                if ($rs){
                    $num=@bdd_pg_num_rows($rs);
                        ?>
                        <p class="ok"> Borrado exitosamente.</p>
                        <p > Regresar a <a href="/indicadores/catalogos/responsable_dato/">Responsable de Dato</a></p>
                        <?php
                } else {
                        ?>
