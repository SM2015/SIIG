<?php
session_start();
include_once("responsable_dato.php");
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        <title>Catalogo Responsable de Datos</title>
	...
	<table  border="0" cellpadding="2" cellspacing="0" class="dataTable" width="100%">
        ...
        switch ($accion) {
                case 'nuevo':
                        agregar_responsable_dato();
                break;
                case 'guardarnuevo':
                        grabar_nuevo_responsable_dato();
                break;
                case 'editar':
                        $id = (isset($_REQUEST['id']))? $_REQUEST['id'] : NULL;
                        editar_responsable_dato($id);
                break;
                case 'guardar':
                        $id = (isset($_REQUEST['id']))? $_REQUEST['id'] : NULL;
                        actualizar_responsable_dato($id);
                break;
                        case 'borrar':
                        $id = (isset($_REQUEST['id']))? $_REQUEST['id'] : NULL;
                        borrar_responsable_datos($id);
                break;
                default: ?>
                        <h2 align="center"><a href="index.php?action=nuevo" title="Agregar Responsable de Datos">Agregar Responsable de Datos</a></h2>	<?php
                        $data = array('Editar','Borrar');
                        $urls = array('Editar','Borrar');
                        echo listarTodos('responsable_dato', $data, $urls, 'id_responsable_dato, nombre, contacto, correo, telefono, cargo');
                break;
        }?>
    </body>
</html>
