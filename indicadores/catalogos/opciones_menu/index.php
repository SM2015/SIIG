<?php
session_start();
include_once("opciones_menu.php");
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<table  border="0" cellpadding="2" cellspacing="0" class="dataTable" width="100%">
        ...
        switch ($accion) {
                case 'nuevo':
                        agregar_opciones_menu();
                break;
                case 'guardarnuevo':
                        grabar_nuevo_opciones_menu();
                break;
                case 'editar':
                        $id = (isset($_REQUEST['id']))? $_REQUEST['id'] : NULL;
                        editar_opciones_menu($id);
                break;
                case 'guardar':
                        $id = (isset($_REQUEST['id']))? $_REQUEST['id'] : NULL;
                        actualizar_opciones_menu($id);
                break;
                        case 'borrar':
                        $id = (isset($_REQUEST['id']))? $_REQUEST['id'] : NULL;
                        borrar_opciones_menu($id);

                break;
                default: ?>
                        <h2 align="center"><a href="index.php?action=nuevo" title=" Agregar Opciones de Menu">Agregar Opciones de Menu</a></h2>	<?php
                        $data = array('Editar','Borrar');
                        $urls = array('Editar','Borrar');
                        echo listarTodos('opciones_menu', $data, $urls, 'id_menu, nombre, id_padre, ruta, publicado');
                break;
        }?>
    </body>
</html>
