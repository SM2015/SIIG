<?php
function agregar_fuente_dato(){
$cnx=bdd_conectar(); ?>
<form action="<?php echo $_SERVER['PHP_SELF']?>?action=guardarnuevo" method="POST" id="add" name="add">
<h3 class="frm-title-x">Agregar una Fuente de Dato</h3>
	<fieldset ><legend> Ingreso de una Fuente de Dato</legend>
	<table width="100%" align="center">
		<TR class="frm-fld-x-odd">
			<TD width="20%">Nombre del Fuente del Dato</TD>
			<TD width="80%">
...
function grabar_nuevo_fuente_dato(){
$cnx = bdd_conectar();
...
$q = "INSERT INTO fuente_dato ( nombre, contacto, correo, telefono, cargo )
    VALUES (
  '".$nombre."' ,
  '".$contacto."' ,
  '".$correo."' ,
  ...
  '". $cargo ."')";
	...
	if ($af){ 
		...
		<p > Regresar a <a href="/indicadores/catalogos/fuente_dato/">Fuente del Datos</a></p>
		<?php
	} else { 
		?>
		...
		if ($num > 0) { 
			...
			while ($i < $numf  ) {
				...
				switch ($fname) {
					...
					break;
					case 'nombre':
						$fname = "Nombre del establecimiento o dependencia Fuente del dato";
					break;
					case 'contacto':
...
function editar_fuente_dato($id){
	$cnx = bdd_conectar();
	$q = '	SELECT id_fuente_dato, nombre, contacto, correo, telefono, cargo
  FROM fuente_dato where id_fuente_dato='.$id;
	$rs = bdd_pg_query($cnx, $q);
	if ($rs) { 
		$reg = bdd_pg_fetch_row($rs)
		?>
		<h3>Editar Una Fuente de Dato</h3>
		<form action="<?php echo $_SERVER['PHP_SELF']?>?action=guardar" method="POST" id="edit" name="edit" >
			<fieldset ><legend> Editar Una Fuente de Dato </legend>
	<table width="100%" align="center">
		<TR class="frm-fld-x-odd">
			<TD width="20%">Nombre de la Fuente del Dato</TD>
			<TD width="80%">
...
function actualizar_fuente_dato($id){
	$cnx = bdd_conectar();
	$nombre =(isset($_POST['nombre'])) ? $_POST['nombre'] : "";
	$contacto =(isset($_POST['contacto'])) ? $_POST['contacto'] : "";
	$correo =(isset($_POST['correo'])) ? $_POST['correo'] : "";
	$telefono =(isset($_POST['telefono'])) ? $_POST['telefono'] : "";
	$cargo =(isset($_POST['cargo'])) ? $_POST['cargo'] : "";
	$q = "UPDATE fuente_dato SET
			nombre= 	'".$nombre."'
		,	contacto= 	'".$contacto."'
		,	correo= 	'".$correo."'
		,	telefono= 	'".$telefono."'
		,	cargo= 		'".$cargo."'
		where 	id_fuente_dato=".$id;
	$rs = bdd_pg_query($cnx, $q);
	$af = bdd_pg_affected_rows($rs);
	if ($af){ 
		?>
...
function borrar_fuente_datos($id){
	if ($id == NULL) {
		?>
		...
		$cnx = bdd_conectar();
                $q = "DELETE FROM fuente_dato WHERE id_fuente_dato=".$id;
                $rs = @bdd_pg_query($cnx, $q);
                if ($rs){
                    $num=@bdd_pg_num_rows($rs);
                        ?>
                        <p class="ok"> Borrado exitosamente.</p>
                        <p > Regresar a <a href="/indicadores/catalogos/fuente_dato/">Fuente de Dato</a></p>
                        <?php
                } else {
                        ?>
