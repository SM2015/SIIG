<?php
function agregar_clas_nivel(){
$cnx=bdd_conectar(); ?>
<form action="<?php echo $_SERVER['PHP_SELF']?>?action=guardarnuevo" method="POST" id="add" name="add">
<h3 class="frm-title-x">Agregar una Categoria de clasificacion segun su Nivel</h3>
	<fieldset ><legend> Ingreso de una Categoria de clasificacion segun su Nivel</legend>
...
frmvalidator.addValidation("descripcion","req","Nombre de la Categoria de clasificacion segun su nivel es requerida");
</script>
 <?php
bdd_cerrar($cnx);
}

function grabar_nuevo_clas_nivel(){
$cnx = bdd_conectar();
$descripcion =(isset($_POST['descripcion'])) ? $_POST['descripcion'] : "";
$comentario =(isset($_POST['comentario'])) ? $_POST['comentario'] : "";

$q = "INSERT INTO clasificacion_nivel ( descripcion, comentario )
    VALUES (
  '".$descripcion."' ,
  '".$comentario."')";
	...
	if ($af){ 
		...
		<p > Regresar a <a href="/indicadores/catalogos/clasificacion_nivel/">Clasificacion segun su nivel</a></p>
		<?php
	} else { 
		?>
...
function editar_clas_nivel($id){
	$cnx = bdd_conectar();
	$q = '	SELECT id_clasificacion_nivel, descripcion, comentario
  FROM clasificacion_nivel where id_clasificacion_nivel='.$id;
	$rs = bdd_pg_query($cnx, $q);
	if ($rs) { 
		$reg = bdd_pg_fetch_row($rs)
		?>
		<h3>Editar una Categoria de clasificacion segun su Nivel</h3>
		<form action="<?php echo $_SERVER['PHP_SELF']?>?action=guardar" method="POST" id="edit" name="edit" >
			<fieldset ><legend> Editar una Categoria de clasificacion segun su Nivel</legend>
	<table width="100%" align="center">
		<TR class="frm-fld-x-odd">
			<TD width="20%">Categoria de clasificacion</TD>
			<TD width="80%">
...
function actualizar_clas_nivel($id){
	$cnx = bdd_conectar();
$descripcion =(isset($_POST['descripcion'])) ? $_POST['descripcion'] : "";
$comentario =(isset($_POST['comentario'])) ? $_POST['comentario'] : "";
$q = "UPDATE clasificacion_nivel SET
			descripcion= 	'".$descripcion."'
		,	comentario= 	'".$comentario."'
		where 	id_clasificacion_nivel=".$id;
	$rs = bdd_pg_query($cnx, $q);
	$af = bdd_pg_affected_rows($rs);
	if ($af){ 
		?>
...
function borrar_clas_nivel($id){
	if ($id == NULL) {
		...
		$cnx = bdd_conectar();
                $q = "DELETE FROM clasificacion_nivel WHERE id_clasificacion_nivel =".$id;
                $rs = @bdd_pg_query($cnx, $q);
                if ($rs){
                    $num=@bdd_pg_num_rows($rs);
                        ?>
